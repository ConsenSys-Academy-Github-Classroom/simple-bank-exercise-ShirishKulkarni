...
contract SimpleBank {
    ...
    event UserNotEnrolled(address  accountAddress);

// Add an argument for this event, a balance
    ...
    event NotEnoughBalance(address  accountAddress, uint amount);
    ...
    function deposit() public payable returns (uint) {
      ...
      if(enrolled[msg.sender]) {
        ...
        uint curBalance = balances[msg.sender];
        curBalance = curBalance + msg.value; 
        balances[msg.sender] = curBalance;
        // 4. Emit the appropriate event associated with this function
        emit LogDepositMade(msg.sender,curBalance);
        // 5. return the balance of sndr of this transaction
        return curBalance;

      } else {
        emit UserNotEnrolled(msg.sender);
        revert();
      }
    ...
    function withdraw(uint withdrawAmount) public payable returns (uint) {
      ...
      //    sender's balance
            uint newBalance = curBalance - withdrawAmount;
            balances[msg.sender] = newBalance;
            // 3. Emit the appropriate event for this message
            emit LogWithdrawal(msg.sender, withdrawAmount, newBalance);
            return newBalance;
         } else{
           require(curBalance>=withdrawAmount , "Balance too low. Not enough funds.");
            emit NotEnoughBalance(msg.sender,curBalance);
            revert();
         }
